...
-   🏛️ **Organizations:** Worked with **JamboPay,Girwa-Foundation, Dubu Huru Ltd, Universities Fund, Caava Group**.
...
| :--- | :--- |
| **Backend Languages** | Java(SpringBoot), Python (Django, DRF, Fast API, Flask), C++  |
...
| **Linka** | Linka is a transparent giving platform that connects donors to verified beneficiaries in real-time. | `Python`, `Django`, `DRF` , `TypeScript`,`PostgreSQL` ,`MPESA` ,`Paystack`, `AWS ECS` ,`ElastiCache`, `Celery`, `Docker`,`AWS S3`,`AWS Dynamo DB`, `Github Actions(CI/CD)`, `Terraform(IaC)` |
| **spotter.ai.** | An assessment test to build a minimalistic interface with **Open Street Map** for drivers to calculate optimal routes and efficient fuel stations. | `Python`,`Fast API` , `PostgreSQL` |
